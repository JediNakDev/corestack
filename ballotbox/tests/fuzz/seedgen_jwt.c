/*
 * seedgen_jwt.c - mints the JWT seed corpus.
 *
 * A separate program because a valid HS256 token cannot be written by hand
 * into a shell script, and a coverage-guided mutator will never produce one:
 * without a real token to mutate from, fuzz_jwt_verify explores the "not three
 * segments" and "bad base64" branches forever and never reaches claim
 * validation or expiry, which is where the interesting logic is.
 *
...
int main(int argc, char **argv) {
  ...
  struct {
    ...
    jwt_claims_t claims;
      ...
      /* Boundary: exp exactly at now. RFC 7519 s4.1.4 is "on or after", so
  ...
  for (size_t i = 0; i < sizeof cases / sizeof cases[0]; i++) {
    ...
    if (jwt_mint(token, sizeof token, &cases[i].claims, JWT_FUZZ_SECRET,
