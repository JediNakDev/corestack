...
/* The tables this daemon creates on start.
 *
 * "user" is here unconditionally: every project that runs tetrisdb also runs
 * libtetrisauth against it. Everything after it is the owning project's
 * business - tetriSH wants "history", ballotbox wants nothing - which is what
 * DB_PROVISION_TABLES carries in from that project's own
 * include/tetrisdb/provision.h. */
static const struct
{
    const char *table;
    ...
    {TETRISAUTH_DB_TABLE, TETRISAUTH_DB_SCHEMA},
    DB_PROVISION_TABLES
};

static int provision(const db_runner_opts_t *opts)
{
    for (size_t i = 0; i < sizeof provision_tables / sizeof provision_tables[0];
         i++)
    {
        if (db_ensure_table(opts->dir, provision_tables[i].table,
                            provision_tables[i].schema) != 0)
        {
