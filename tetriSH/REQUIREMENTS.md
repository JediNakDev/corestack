...
![tetriSH](https://natalieagus.github.io/50005/docs/Programming%20Assignment/images/TetriSH/2026-05-15-18-48-24.png)
...
A group that takes tetriSH does **not** need to separately submit PA1 and PA2.

Instead, the group submits one integrated system that demonstrates the required PA1 and PA2 competencies through the tetriSH architecture. A polished game interface alone is **not** sufficient, and this is an assessment on systems engineering. You will be interviewed and asked to implement live features during the checkoff.

The project component remains **capped** at the same mark as the normal PA1 + PA2 path. There are **no additional bonus marks** for attempting tetriSH. However, there's a **prize** for the top 2 groups.
...
`tetriSH` is a single integrated project that combines [PA1](https://natalieagus.github.io/50005/pa1/intro) (shell + daemon) and [PA2](https://natalieagus.github.io/50005/pa2/intro-c) (authenticated, confidential client-server protocol) into one **terminal-based** Battle-Royale Tetris system written entirely in C.
...
> **Required**
>
> `tetrislogd` is a separate **process**, not a thread inside `tetrisd`. It receives log records over an IPC channel and writes them to disk.
...
>
> Cryptographic primitives come from PA2's `common.c`. You are **not** allowed to modify `common.c` or use any cryptographic library other than OpenSSL.
...
**We shall use `HTTTP/1.0` literal.**
...
> **Required**
>
> `tetrisctl` and `tetrisd` communicate via a real IPC mechanism, not the public TCP socket and not function calls. The mechanism must be **local-only**.
>
> The control plane must remain available even when the public TCP listener is congested. Imagine the server is being overwhelmed by client requests and you need to issue `tetrisctl shutdown`. **It must still work.**
...
> Why so complex?
>
> This pattern matches production loggers (`systemd-journald` drops with `Suppressed N messages` lines when overwhelmed). Your system does not need to be identical, but the principles (non-blocking on the producer, observable drops, separate process for the writer) are **required**.
...
Battle Royale mode is a **required** part of tetriSH.
...
The baseline MVP exists to define the minimum integrated system that must work **before** Battle Royale can be meaningfully assessed. A group that cannot demonstrate the baseline MVP **cannot** pass tetriSH, even if some Battle Royale code exists.
...
Members may help one another. That is expected. However, each member **must be able to answer live Q&A questions** about their declared role.
...
The project **does not need** to be the most feature-rich submission to receive full project marks. It must be correct, demonstrable, understandable, and aligned with the required systems objectives.
...
Before this deadline, a group **must** notify both the 50.005 and 50.003 instructors that it is _withdrawing_. The group will then complete PA1 and PA2 in the normal way and will be graded using the normal PA1 and PA2 rubrics on the 50.005 side.
...
These multipliers would **compound**, not add.
...
A group that submits working AI-generated code but cannot explain it may lose **immediate 10% penalty** and will not be eligible for the prize.

## Prize eligibility

The prize is **separate** from course marks. There will be a live demo session for shortlisted groups in Week 13, stay tuned.
...
8. ship a clean release bundle from a tagged commit (build instructions, sample `.tetrishrc`, certificates, install steps) that **any other group in the cohort** can clone and run end to end without your assistance,
...
> SUTD Wifi
>
> It is your responsibility to check how clients can connect to the server's private IP via SUTDWifi. Otherwise, you may also host it somewhere and ssh into it. See [this](https://natalieagus.github.io/50005/pa2/intro-c) for more info.
...
The prize is **not** awarded purely for the most features. Basically, a smaller but cleaner and better-understood system may beat a larger but fragile system.
