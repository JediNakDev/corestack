...
int main(void)
{
    ...
    {
        fprintf(stderr,
                COLOR_RED "backup: BACKUP_DIR environment variable is not set.\n" COLOR_RESET);
        fprintf(stderr,
    ...
    {
        fprintf(stderr, COLOR_RED "backup: cannot access '%s': %s\n" COLOR_RESET,
    ...
    {
        fprintf(stderr, COLOR_RED "backup: cannot create '%s': %s\n" COLOR_RESET,
    ...
    snprintf(archive_path, sizeof(archive_path), "%s/%s_%s.zip",
             archive_dir, base, timestamp);

    /* Build the zip command. `zip -r -q` handles directories; for a
       single file the -r flag is harmless. */
    char command[PATH_MAX * 3];
    snprintf(command, sizeof(command),
             "zip -r -q '%s' '%s'", archive_path, backup_target);

    int rc = system(command);
    if (rc == -1 || WEXITSTATUS(rc) != 0)
    {
        fprintf(stderr, COLOR_RED "backup: failed to create archive for '%s'\n" COLOR_RESET,
                backup_target);
        return EXIT_FAILURE;
    }
