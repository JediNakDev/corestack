#include "lib/rc_parser.h"
#include "shell.h"

#include <ctype.h>

static shell_status_t run_builtin(char **cmd)
{
...
static void execute_command_line(char *line)
{
    ...
    char *token = strtok(line, SHELL_DELIMS);
...
static void process_rc_file(const char *shell_dir)
{
    ...
    while (fgets(line, sizeof(line), rc) != NULL)
    {
        ...
        if (type == RC_LINE_EMPTY || type == RC_LINE_DIRECTIVE)
        {
            continue;
        }
...
int main(void)
{
    ...
    if (setenv("TETRISH_ROOT", shell_dir, 1) != 0)
        perror("setenv TETRISH_ROOT");
    ...
    for (;;)
    {
        ...
        // Ctrl-D at the prompt ends the session, the same as `exit`.
        if (read_command(cmd) == SHELL_EOF)
        {
            printf("\n");
            break;
        }
        ...
        shell_status_t builtin_result = run_builtin(cmd);
